...
- LMS at https://mooc.academiacentral.org
- CMS at https://studio.mooc.academiacentral.org
- CDN at https://cdn.mooc.academiacentral.org linked to a public read-only S3 bucket named mooc-academiacentral-global-storage
- public ssh access via a t2.micro Ubuntu 20.04 LTS bastion EC2 instance at bastion.mooc.academiacentral.org
...
- LMS at https://dev.mooc.academiacentral.org
- CMS at https://studio.dev.mooc-academiacentral.org
- CDN at https://cdn.dev.mooc.academiacentral.org linked to an S3 bucket named dev-academiacentral-global-storage
...
.. code-block:: shell
  ...
  ssh bastion.mooc.academiacentral.org -i path/to/uedx.pem

  mysql -h mysql.mooc.academiacentral.org -u root -p

  mongo --port 27017 --host mongo.master.mooc.academiacentral.org -u root -p
  mongo --port 27017 --host mongo.reader.mooc.academiacentral.org -u root -p

  redis-cli -h redis.primary.mooc.academiacentral.org -p 6379
...
- **MySQL**. uses `AWS RDS <https://aws.amazon.com/rds/>`_ for all MySQL data, accessible inside the vpc as mysql.mooc.academiacentral.org:3306. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/environments/prod/rds/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.mooc.academiacentral.org:27017 and mongodb.reader.mooc.academiacentral.org. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/modules/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **Redis**. uses `AWS ElastiCache <https://aws.amazon.com/elasticache/>`_ for all Django application caches, accessible inside the vpc as cache.mooc.academiacentral.org. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_. This is necessary in order to make the Open edX application layer completely ephemeral. Most importantly, user's login session tokens are persisted in Redis and so these need to be accessible to all app containers from a single Redis cache. Common configuration settings `are located here <terraform/environments/prod/redis/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
...
- **CDN**. uses `AWS Cloudfront <https://aws.amazon.com/cloudfront/>`_ as a CDN, publicly acccessible as https://cdn.mooc.academiacentral.org. Terraform creates Cloudfront distributions for each of your enviornments. These are linked to the respective public-facing S3 Bucket for each environment, and the requisite SSL/TLS ACM-issued certificate is linked. Terraform also automatically creates all Route53 DNS records of form cdn.mooc.academiacentral.org. Common configuration settings `are located here <terraform/environments/prod/cloudfront/terragrunt.hcl>`_
